...
RESEARCH_MODELS = ('claude', 'gemini', 'qwen', 'deepseek-flash', 'deepseek-pro')  # gemma off — code kept (remote worker, completion handling) so it can be re-enabled by adding 'gemma' back. deepseek-flash/pro: research_multimodel.py via DeepSeek's Anthropic-compat endpoint, web context via SerpAPI prefetch
