...
RESEARCH_MODELS = ('claude', 'gemini')  # gemma and qwen off — code kept (remote worker, completion handling) so either can be re-enabled by adding it back. qwen off 2026-06-08: the Mac remote worker was offline and qwen jobs accumulated staged. chatgpt (the compatibility key for the subscription-backed Codex CLI peer launched by research_multimodel.py) off 2026-09-19: Codex subscription tokens are reserved for other work; re-enable by adding it back. deepseek-flash/pro off 2026-06-29 — code kept in research_multimodel.py so either can be re-enabled by adding it back. claude re-enabled 2026-07-16 under the hard 3-subagent PreToolUse cap (TJAI_MAX_SUBAGENTS), after the 2026-07-15 run (74 subagents) exhausted host memory.
