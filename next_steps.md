...
## Bulk Bookmark Import (Ready for Deployment)

A bulk import system for bookmarks from external sources (Dynalist, Pinboard, etc.).
...
Desktop                              Server
───────                              ──────
bulk_import/client.py                /api/bulk-import
    │                                     │
    ├──► POST chunk (100 items) ────────► bulk_import/loader.py
    │                                     │
    │    ◄── {imported, skipped} ◄───────┘
    │                                     │
    ...                              (auto-tagger runs)
...
- `bulk_import/` - Module directory
  - `__init__.py` - Module init
  - `loader.py` - Server-side bulk loader (Django ORM + auto-tagger)
  - `client.py` - Desktop client with chunking
  - `parsers/` - Source-specific parsers
    - `dynalist.py` - Dynalist .txt/.zip parser

- `tjai_app/views.py` - Added `api_bulk_import` endpoint
- `tjai_project/urls.py` - Added URL pattern

### Deployment Steps

1. **Deploy to server**:
   ```bash
   cd ~/github/tjrepo/tjai
   ./deploy/update_from_dev.sh
   ```

2. **Get API key** (same as Chrome extension):
   ```bash
   # On server, or via tj CLI
   tj admin config gmail_addon_api_key
   ```

3. **Run import from desktop**:
   ```bash
   cd ~/github/tjrepo/tjai
   ...
   # Import from Dynalist zip
   python3 bulk_import/client.py ~/Desktop/dynalist-backup-2026-02-12.zip \
       --source-tag dynalist

   # Or from JSON
   python3 bulk_import/client.py ~/Desktop/dynalist-bookmarks.json \
       --source-tag dynalist
   ```

### Parser Output Format

Parsers output a list of dicts:
```json
{
  "content": "[Title](url)",
  "tags": ["recipe", "nyer"],
...
- **Dynalist parser**: Tested with 4,824 bookmarks
- **Server endpoint**: Code ready, needs deployment
- **Client**: Tested in dry-run mode
- **Auto-tagger**: Will apply rules (youtube→video, github→github, recipe→recipe, etc.)
