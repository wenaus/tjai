...
2. One model pass (the `picks-agent` action's `model`; opus since 2026-09-19) judges that file against the user profile. It may verify only a small finalist set on the web, then creates the selected bookmark entries with precis and rationale.
...
"Computer, perform an analysis." Deep autonomous research — the system dispatches **up to six models in parallel** (Claude, Gemini, ChatGPT, Qwen on Torre's Mac Studio via the [Remote Worker Pipeline](remote-workers.md), and DeepSeek-Flash + DeepSeek-Pro via DeepSeek's Anthropic-compatible endpoint). Each model produces an independent analyst's brief, then a final synthesis pass merges them into a single report.
...
5. **Synthesis** — the research-agent action is dispatched again with the synthesis prompt and links to all per-model reports (a failed branch is listed and its entry holds the error), producing the final merged analyst's brief. It runs on the action's own `model` and `effort` unless the action's data sets `synthesis_model` / `synthesis_effort` (e.g. `gpt-5.6-sol` to synthesize outside the Claude family)
...
autodiscovers each topic/model/synthesis entry-page link from those detail
payloads. Missing linked entry pages are cached in Cache Storage so the normal
report and synthesis links load offline. It registers `/tjai/research-sw.js` so
the research shells and cached linked report pages load offline. Online
...
actions remain live-server operations. Claude subagent studies are not cached.
