...
3. Click **Submit** or **Run**. The research-agent action runs `_dispatch_research_3way` (`tjai_app/action_runner.py`), which dispatches each model in `RESEARCH_MODELS` (currently `('gemini', 'chatgpt')`; claude, gemma, qwen, and deepseek are off with code retained for re-enable — see the tuple's comment in `action_runner.py` for per-model dates and reasons):
   - **Claude** (off 2026-07-15: a research run's parallel subagents exhausted host memory) — detached Claude instance with research-optimized system prompt, spawning parallel subagents that search the web and write findings tagged `research-subagent`. The `tj agent` harness now hard-caps subagent spawns at 3 via a PreToolUse hook (see [action-agent.md](action-agent.md) § AI Dispatch); re-enabling claude here is a one-word add to `RESEARCH_MODELS`
