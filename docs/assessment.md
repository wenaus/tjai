...
The daily AI performance assessment scores a day's human–AI dialog against the rules in the `assessment-system-prompt` entry and writes one entry per day, which the assessment dashboard reads. The assessor since 2026-09-18 is DeepSeek-V4.1-Flash (`deepseek-flash`) through DeepSeek's Anthropic-compatible API, thinking at effort `high` (the levels are low, high, max), no tools, one call per pack; it is the model DeepSeek serves for every id since 2026-09-14, ahead of V4-Pro on their own and third-party tests. From 2026-09-06 to 2026-09-17 it was `gpt-5.6-sol` through the Codex subscription; that path is kept (`--provider codex`) and was left when the Codex usage limit stopped it on 2026-09-19. A Claude path exists (`--provider claude`, opus/xhigh) and is not the assessor: Opus 5's API safeguards refused the 2026-09-18 pack as `[cyber]` for the day's IAM-key dialog. Both are outside the family that supplies most of the assessed dialog. Action `llm-assessment-gemini`, daily at 02:20 for the previous day, script `scripts/assessment_gemini.py`.

The entry id is `assessment-<date>-<assessor>`, so a change of assessor leaves the previous reader's history under its own name rather than overwriting it: days before 2026-09-06 end in `-gemini`, days from 2026-09-06 in `-sol`, days from 2026-09-18 in `-deepseek`. The dashboard plots the assessors named in `ASSESSOR_SUFFIXES` (`tjai_app/views.py`); assessments written before the suffix existed are not plotted.
