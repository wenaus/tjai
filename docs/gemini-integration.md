...
This document records the integration of the **Antigravity CLI** (`agy`) into Torre's multi-CLI mix and describes how it is configured to match his **Claude Code** and **Codex** environments.
...
The integration delivers an automated, secure setup that supports:
1. **Auto-approvals for passive info-gathering commands only** (via `settings.json`)
2. **Dynamic startup context loading (recent dialog, profile, and guidance)**
3. **Availability of the `tjai` MCP server and tools**
4. **Self-logging of conversational turns back to `tjai`**
...
```mermaid
graph TD
    User([User types 'agy']) --> Wrapper[~/.bashrc wrapper function]
    ...
    Wrapper -->|Launch 'agy'| CLI[antigravity-cli executable]
...
All active, state-changing, or risky operations (e.g., file edits, deletions, git push) still require your explicit prompt approval.

### 2. Startup Context & Dialog Loader (`gemini_load.py`)
Since the Antigravity CLI does not support session-start shell hooks like Claude, we created a lightweight pre-launch wrapper script:
...
* **Why this works**: The Antigravity CLI automatically reads `GEMINI.md` from the workspace root on startup and appends it to hierarchical system instructions. This ensures the model boots up with full context and memories instantly.

### 3. Launch Function (`~/.bashrc`)
An `agy()` wrapper function is registered directly below the `codex()` function in the tracked `~/.bashrc` file (`tjrepo/computers/laptop/config-files/.bashrc`):
```bash
agy() {
...
* **Why this is critical**: 
  ...
  * It executes the default `agy` command, ensuring that permissions remain strictly governed by the fine-grained passive-only rules in `settings.json`.

### 4. `tjai` MCP Server Stdio Bridge (`tjai_mcp_bridge.js`)
We registered your `tjai` HTTP MCP server in `settings.json`. Since the Antigravity CLI does not support custom header injection (like Claude's `"headers": {"Authorization": "Bearer ${TJAI_MCP_TOKEN}"}`), we built a high-performance Stdio-to-POST bridge:
...
* **Registered in Settings**:
  ...
  ```
* **Function**: Communicates over `stdio` with the Antigravity CLI, translates all outgoing JSON-RPC requests, and sends them via local Node `fetch` POST requests directly to `https://etaverse.com/tjai/mcp/` with your Bearer Token, returning results instantly back to stdout.

### 5. In-Session Conversational Turn Self-Logging (`record_turn.py`)
To ensure that conversational turns in Antigravity sessions are saved back to the `tjai` dialog store:
...
* **Permissions Advantage**: Because the CLI operates with auto-approvals for passive commands (like executing the python logging script), this self-logging command executes instantly and silently in the background, keeping cross-session memory perfectly synchronized.
