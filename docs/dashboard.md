...
`fetchCalendar()` initially fetches `api/dashboard/calendar` with no parameters and `renderCalendar()` builds the list. The client keeps the loaded calendar window in page memory only. Scroll near the top loads the previous 30-day slice with `before=<start_ts>` and preserves scroll position; scroll near the bottom loads the next 30-day slice with `after=<end_ts>`. Dashboard auto-refreshes reuse the expanded window with `start=<start_ts>&end=<end_ts>` so the calendar does not collapse back to the default range while the page remains open. A browser reload, new visit, or navigation away and back starts again from the default range.

An auto-refresh rebuilds the list only when the markup it produces differs from what is already displayed; the comparison is against the rendered HTML, held in `calendarHtmlKey`. An unchanged calendar is left untouched, so scroll position is unaffected by a refresh that brings no new events. When the list is rebuilt, position is restored against the day header that was at the top of the view (`data-day`) rather than a pixel offset, because the heights above the viewport change as events appear and disappear. Replacing the list resets `scrollTop` and the browser dispatches a scroll event for it, which is indistinguishable from the user reaching the top; programmatic scrolling is therefore bracketed by `suppressCalendarScroll()` and the scroll listener ignores events raised while it is active, so the older-page loader responds only to real scrolling. The server caps expansion to 180 days outside the default window and returns `has_older` / `has_newer` so the scroll loader stops at the bounds. The server returns events with pre-formatted Eastern-time fields (`date_display`, `time_display`, `date_key`, `week_num`, `week_start_key`) so the client does no date math for display.

Behavior:

- If the current day has no events, a placeholder is inserted so a "Today" marker always exists. On first render the list scrolls to put today a third of the way down; later refreshes preserve scroll position. The "Today" link in the calendar panel header re-centers without reloading.
- The next future event gets a countdown chip. The chip stays on a just-started event and counts negative for up to 15 minutes after its start (`LATE_GRACE_SEC`). Its text advances with the clock rather than with the data, so it is rendered into an element of its own (`#cal-countdown`) and updated in place; the surrounding markup carries an empty placeholder for it, keeping the ticking text out of the comparison that decides whether the list is rebuilt.
...
A refresh re-renders the list only when the entry payload differs from the one last rendered, compared as JSON in `lastEntriesJson`. A poll that returns the same entries leaves the list in place.
...
Off-screen rows in this list skip layout through `content-visibility: auto` with `contain-intrinsic-block-size: auto 44px`, which is worth roughly 10ms per render at the 1000-row page size. The rule is scoped to `#entries-list .entry` and must stay scoped there. Calendar rows carry the same `.entry` class but have no fixed height, so a placeholder size stands in for the real one: a rebuilt calendar lays out at true height for one frame and then collapses to the placeholder heights, and any scroll position set in between is computed against a layout that no longer exists. The entries list tolerates this because it is re-rendered only when its content changes and does not restore scroll position.
